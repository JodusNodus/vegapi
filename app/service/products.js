...
module.exports.paginateAll = async function({
  searchquery,
  size,
  page,
  orderby,
  ...
  if (searchquery) {
    stmt.where(function() {
      this.whereRaw(`MATCH(p.name) AGAINST (? IN NATURAL LANGUAGE MODE)`, [
        searchquery.toLowerCase()
      ]).orWhereRaw(`MATCH(b.name) AGAINST (? IN NATURAL LANGUAGE MODE)`, [
        searchquery.toLowerCase()
      ]);
    });
  }

  const totalStmt = stmt
    .clone()
    .countDistinct("p.ean as count")
    .first();
...
const getProductRatingStmt = ean =>
  knex("userratings")
...
module.exports.insertProduct = async function({
  ean,
  name,
  brandid,
  creationdate,
